  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center gap-3 py-12 text-center">
        ...
        <h1 className="font-display text-xl font-bold">Eko-Puan Cüzdanı</h1>
        <p className="max-w-sm text-sm text-muted-foreground">
          Eko-Puan bakiyeni görmek ve Esenlink hesabınla eşleştirmek için giriş
          yap.
        </p>
        <Link to="/auth">
          <Button size="sm">
            <LogIn className="mr-1.5 h-4 w-4" /> Giriş Yap
          </Button>
        </Link>
      </div>
    );
    ...
    <div className="flex flex-col gap-5 py-4">
      <div className="anim-fade-up">
        ...
        <p className="text-sm text-muted-foreground">
          Eko-Puan yalnızca ürün veren kullanıcıya yüklenir. Puanların Esenlink
          sistemine aktarılır.
        </p>
      </div>
      ...
      <section className="rounded-2xl border bg-card p-4">
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          ...
          <Tile
            label="Tahmini önlenen atık"
            value={`${(completedGives * 1.4).toFixed(1)} kg`}
          />
          <Tile
            label="Tahmini tasarruf"
            value={`₺${completedGives * 180}`}
          />
        </div>
      ...
      <section className="rounded-2xl border bg-card p-4">
        ...
        <p className="mt-3 text-xs text-muted-foreground">
          Esenlink entegrasyonu demo modunda gösterilmektedir. Puan bakiyen
          tamamlanan teslimattan sonra Esenlink hesabına otomatik aktarılır.
        </p>
      </section>
      ...
      <section className="rounded-2xl border bg-warning/10 p-4 ring-1 ring-warning/20">
        ...
        <ul className="space-y-1 text-xs text-muted-foreground">
          ...
          <li>
            • Belediye gerekli durumlarda şüpheli puanları incelemeye alabilir.
          </li>
        </ul>
      </section>
      ...
      <section>
        ...
        <div className="divide-y rounded-2xl border bg-card">
          ...
          {!isLoading && history.length === 0 && (
            <div className="p-6 text-center text-sm text-muted-foreground">
              Henüz Eko-Puan kazanmadın. Kullanmadığın bir ürünü paylaşarak
              başlayabilirsin.
            </div>
          )}
          {history.map((t) => (
